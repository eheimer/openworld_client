...
namespace Openworld
{
  public class Communicator : MonoBehaviour
  {
    ...
    // Autogenerated 3/22/2022 10:21:41 AM
    //   by GenerateCommunicator.
    public void CreateGame(string name, Action<Game> success, Action<RequestException> error)
    {
      var path = "/games";
      var req = getAuthorizedRequest(path);
      req.Body = new CreateGameRequest { name = name };
      RestClient.Post<Game>(req)
      .Then(res => { Debug.Log(res); success(res); })
      .Catch(err => HandleError(err, error));
    }
    ...
    //   by GenerateCommunicator.
    public void CreateCharacter(string gameId, string name, Action<CharacterDetail> success, Action<RequestException> error)
    {
      var path = "/games/{gameId}/characters";
      path = path.Replace("{gameId}", gameId);
      var req = getAuthorizedRequest(path);
      req.Body = new CreateCharacterRequest { name = name };
      RestClient.Post<CharacterDetail>(req)
      .Then(res => { Debug.Log(res); success(res); })
      .Catch(err => HandleError(err, error));
    }
