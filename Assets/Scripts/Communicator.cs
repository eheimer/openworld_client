...
namespace Openworld {
  public class Communicator : MonoBehaviour {
    ...
    //   by GenerateCommunicator.
    public void CreateCharacter(string gameId, string name, int strength, int dexterity, int intelligence, Action<ResponseHelper> success, Action<RequestException> error) {
      var path = "/games/{gameId}/characters";
      path = path.Replace("{gameId}", gameId);
      var req = getAuthorizedRequest(path);
      req.Body = new CreateCharacterRequest { name = name, strength = strength, dexterity = dexterity, intelligence = intelligence };
      RestClient.Post(req)
      .Then(res => { Debug.Log(res); success(res); })
      .Catch(err => HandleError(err, error));
    }
