...
//   by GenerateCommunicator.
    public void CreateCharacter(string gameId, string name, int strength, int dexterity, int intelligence, int movement, Action<ResponseHelper> success, Action<RequestException> error)
    {
      var path = "/games/{gameId}/characters"; 
      path = path.Replace("{gameId}",gameId);
      var req = getAuthorizedRequest(path);
      req.Body = new CreateCharacterRequest { name = name , strength = strength , dexterity = dexterity , intelligence = intelligence , movement = movement  };
      RestClient.Post(req)
      .Then(res => { Debug.Log(res); success(res); })
      .Catch(err => HandleError(err, error));
    }
